...
renamed as (
    select
        ...
        cast("Week_Desc" as varchar(1024)) as week_desc,
        cast("Month_ID" as number) as month_id,
        cast("Month_Name" as varchar(1024)) as month_name,
        cast("Quarter_ID" as number) as quarter_id,
        cast("Quarter_Name" as varchar(1024)) as quarter_name,
...
with_hash as (
    select
        cast(
            {{ dbt_utils.generate_surrogate_key(["date_id"]) }} as varchar(1024)
        ) as date_id,
        transaction_date,
        week_id,
        week_desc,
