...
renamed as (
    select
        ...
        cast("transaction_time" as varchar(1024)) as transaction_time,
        cast("sales_outlet_id" as number) as sales_outlet_id,
        cast("staff_id" as number) as staff_id,
        cast("customer_id" as number) as customer_id,
        cast("instore_yn" as varchar(1024)) as instore_yn,
        ...
        cast("promo_item_yn" as varchar(1024)) as promo_item_yn
    from source
),
...
with_hash as (
    select
        cast(
            {{ dbt_utils.generate_surrogate_key(["transaction_id"]) }} as varchar(1024)
        ) as transaction_id,
        transaction_date,
        transaction_time,
        cast(
        ...
        orders,
        cast(
            {{ dbt_utils.generate_surrogate_key(["line_item_id"]) }} as varchar(1024)
        ) as line_item_id,
        cast(
